...
import 'components/multi_items_bottom_sheet_list_builder.dart';

class AppMultiDropDownBox<T> extends StatefulWidget {
  const AppMultiDropDownBox({
    super.key,
    required this.items,
    required this.initSelections,
  ...
  @override
  State<AppMultiDropDownBox<T>> createState() => _AppDropDownBoxState<T>();
}

class _AppDropDownBoxState<T> extends State<AppMultiDropDownBox<T>> {
  late final controller = widget.controller ?? TextEditingController();

  String getText(List<T> items) {
    return items
        .map((e) => widget.selectedItemTitle(e))
        .join(widget.separatorPattern);
  }

  @override
  Widget build(BuildContext context) {
    final String initialTextValue = widget.initSelections != null
        ? getText(widget.initSelections as List<T>)
        : '';
    WidgetsBinding.instance.addPostFrameCallback((_) {
      controller.text = initialTextValue;
    });
            ...
            Expanded(
              child: Text(
                controller.text.isEmpty
                    ...
                    : controller.text,
                style: widget.enabled && controller.text.isNotEmpty
  ...
  Future<void> _selectItem(BuildContext context) async {
    await showModalBottomSheet(
      ...
      builder: (context) {
        final child = MultiItemsBottomSheetListBuilder<T>(
          items: widget.items,
          initSelection: widget.initSelections,
          onItemSelected: (items) {
            setState(() {
              widget.onItemSelected.call(items);
              controller.text = getText(items);
            });
          },
